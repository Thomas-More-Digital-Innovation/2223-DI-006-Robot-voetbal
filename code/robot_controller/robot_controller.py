...
import http.client
postBackward = "http://192.168.10.228/postBackward/"
postForward = "http://192.168.10.228/postForward/"
postTurnLeft ="http://192.168.10.228/postTurnLeft/"
postTurnRight = "http://192.168.10.228/postTurnRight/"
...
class Example(tk.Frame):
    ...
    def on_wasd(self, event):
        ...
        if event.keysym == "w":
            print("w")
            h1.request('POST', '/forward?time=300')
            h1.close()

        elif event.keysym == "a":
            print("a")
            h1.request('POST', '/turnLeft?time=300')
            h1.close()
        elif event.keysym == "s":
            print("s")
            h1.request('POST', '/backward?time=300')
            h1.close()
        elif event.keysym == "d":
            print("d")
            h1.request('POST', '/turnRight?time=300')
            h1.close()
